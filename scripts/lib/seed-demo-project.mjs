/**
 * The demo project the screenshot and store-asset scripts both photograph.
 *
 * Runs inside the extension page via `page.evaluate`, so it must stay
 * closure-free — Playwright ships the function's source across, not its scope.
 * Everything travels through the extension's own messaging layer, the same path
 * a user's data takes; nothing is written straight into IndexedDB.
 *
 * The content is deliberately plausible research rather than "test 1 / test 2":
 * these images are what people see before they install anything.
 *
 * @param {string} pdf - base64 of e2e/fixtures/sample.pdf, for the PDF document.
 */
export const seedDemoProject = async (pdf) => {
