...
CI runs typecheck → lint → **format:check** → unit → build, plus a separate E2E job. Both must be
green on `main`. CI only _checks_ formatting; run `npm run format` yourself. `.prettierignore` keeps
three things out of it: `doc/` (hand-wrapped prose, plus the read-only design mock), `docs/`
(generated by `npm run pages` — Prettier and the generator would overwrite each other) and
`CHANGELOG.md` (released history is append-only; a formatter must not rewrite shipped entries).
