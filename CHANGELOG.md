...
The first release meant for the Chrome Web Store. Everything the store requires and the repo did not
yet have: a hosted privacy policy, images at a size the store accepts, listing copy written out, and
a check that the archive would actually be accepted. No behaviour changes — 1.0.0 marks that all five
roadmap phases have been delivered and the extension is ready for people who did not build it.

### Added

- **Package validation.** `npm run package` now checks the built zip against what the Chrome Web
  Store enforces at upload: MV3 manifest, the 132-character summary limit, a version matching
  `package.json`, all four icons and every manifest-referenced surface present in the archive,
  `manifest.json` at the root, and no source maps, `node_modules`, Vite cache or macOS junk.
  Undeclared permissions warn rather than fail. The rules are a pure module
  (`scripts/lib/store-package-rules.mjs`) with 17 unit tests; the wrapper uses the system `unzip`,
  so no dependency was added.
- **Store listing images** in `doc/store/` — five captioned 1280×800 screenshots, a 440×280 promo
  tile and a 1400×560 marquee, generated by `npm run store:assets`. The store accepts only 1280×800
  or 640×400, so the existing `doc/screenshots/` set (1360×940, and a 400×820 side panel) would have
  been refused outright. Surfaces are captured at 2× device pixels, the side panel is composed onto
  the canvas beside its caption rather than stretched, and every output's dimensions are asserted
  from the PNG header.
- **A published privacy policy** at
  <https://amigouk.github.io/Research-Chrome-Extension/privacy.html>, required for an extension that
  requests broad optional host permissions. `npm run pages` generates `docs/` — a landing page and
  the policy — from `doc/PRIVACY.md`, so there is one source and the published copy cannot drift.
- **`doc/STORE-LISTING.md`** — every dashboard field written out to paste: detailed description,
  single purpose, a justification per permission, the data-usage answers and a note for the reviewer.
- **`homepage_url`** in the manifest, pointing at the repository.

### Changed

- **The manifest description is now store-facing.** It is literally the summary the Web Store prints
  under the name in search results, and it read like internal documentation ("A Chrome MV3 research
  companion…"). Replaced with what the extension does for a researcher, at 129 of the 132 characters
  the store allows.
- `doc/DISTRIBUTION.md` points at the listing copy instead of half-repeating it, and records that the
  store images come from `doc/store/`.
- The demo project the screenshot scripts photograph is now shared
  (`scripts/lib/seed-demo-project.mjs`), so the listing and the README cannot show two different
  products.
